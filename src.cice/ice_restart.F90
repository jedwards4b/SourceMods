...
! author David A Bailey, NCAR
      ...
      if (restart_format == 'pio') then
         ...
         call ice_pio_initdecomp(iodesc=iodesc2d, pio_type=pio_double)
         call ice_pio_initdecomp(ndim3=ncat  , iodesc=iodesc3d_ncat,remap=.true., pio_type=pio_double)
...
! author David A Bailey, NCAR
      ...
      integer (kind=int_kind) :: &
        j,     &      ! dimension counter
        n,     &      ! ncat index
        ndims,  &  ! number of dimensions on file
        status        ! status variable from netCDF routine
